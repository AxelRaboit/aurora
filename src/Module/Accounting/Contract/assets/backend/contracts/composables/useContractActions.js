...
export function useContractActions() {
    ...
    const prefix = "backend.accounting.contracts";
    ...
    function draftActions(contract, handlers) {
        ...
        if (can("accounting.contracts.edit")) {
            actions.push({
                key: "edit",
                icon: Pencil,
        ...
        if (can("accounting.contracts.delete")) {
            actions.push({
                key: "delete",
                color: "rose",
                icon: Trash2,
     ...
     * @param {object} handlers `send`, `revoke`, and `documentPath`
    ...
    function sealedActions(contract, handlers) {
        ...
        if (can("accounting.contracts.send")) {
            if (contract.link) {
                actions.push({
                    key: "revoke",
                    color: "amber",
