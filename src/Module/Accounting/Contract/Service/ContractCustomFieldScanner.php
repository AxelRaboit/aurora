...
final readonly class ContractCustomFieldScanner
{
     ...
     * @return list<string>
     */
    public function keysOf(ContractTemplateVersionInterface $version): array
    {
        $found = [];

        foreach ($version->getTranslations() as $translation) {
            foreach ($this->keysIn($translation->getContent()) as $key) {
                $found[$key] = true;
            }

            foreach ($this->keysInText($translation->getTitle()) as $key) {
                $found[$key] = true;
            }
        }
     ...
     * @return list<string>
     */
    public function keysIn(array $content): array
    {
        ...
        return $this->keysInText((string) json_encode($blocks, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE));
    }

    /** @return list<string> */
    private function keysInText(string $text): array
    {
        if (0 === preg_match_all('/\{\{contract\.custom\.([a-z0-9_]+)\}\}/', $text, $matches)) {
            return [];
        }
