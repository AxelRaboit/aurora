...
class ContractTerminationInput implements ContractTerminationInputInterface
{
    ...
    public function __construct(
        #[Assert\NotBlank(message: 'backend.accounting.contracts.errors.termination_noticed_required')]
        #[Assert\Regex(pattern: '/^\d{4}-\d{2}-\d{2}$/', message: 'backend.accounting.contracts.errors.termination_date_invalid')]
        public readonly string $noticedAt = '',
        #[Assert\NotBlank(message: 'backend.accounting.contracts.errors.termination_effective_required')]
        #[Assert\Regex(pattern: '/^\d{4}-\d{2}-\d{2}$/', message: 'backend.accounting.contracts.errors.termination_date_invalid')]
        public readonly string $effectiveAt = '',
        #[Assert\NotBlank(message: 'backend.accounting.contracts.errors.termination_origin_required')]
        public readonly string $origin = '',
        #[Assert\Length(max: self::MAX_REASON_LENGTH, maxMessage: 'backend.accounting.contracts.errors.termination_reason_too_long')]
