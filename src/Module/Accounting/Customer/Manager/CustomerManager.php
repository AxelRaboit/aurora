...
class CustomerManager implements CustomerManagerInterface
{
    ...
    protected function assertSiretIsFree(?string $siret, ?int $customerId): void
    {
        ...
        throw new FieldException('siret', $this->translator->trans('backend.accounting.customers.errors.siret_taken', ['{name}' => $existing->getLegalName()]));
    }

    protected function auditCreated(CustomerInterface $customer): void
    {
        $this->auditLogger->log('accounting', 'customer.created', 'Customer', $customer->getId(), $this->auditPayload($customer));
    }

    protected function auditUpdated(CustomerInterface $customer): void
    {
        $this->auditLogger->log('accounting', 'customer.updated', 'Customer', $customer->getId(), $this->auditPayload($customer));
    }

    protected function auditDeleted(CustomerInterface $customer): void
    {
        $this->auditLogger->log('accounting', 'customer.deleted', 'Customer', $customer->getId(), $this->auditPayload($customer));
    }
